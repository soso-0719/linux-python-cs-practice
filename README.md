...
- 処理を make_num() と sum_num() に分けた
- time.perf_counter() で処理ごとの実行時間を測定した
- 全体時間だけでなく、処理ごとの時間を見る profiling の考え方を学んだ

## Result

| N | make_numbers | sum_numbers | total |
|---:|---:|---:|---:|
| 1,000,000 | 0.029秒 | 0.017秒 | 0.046秒 |
| 10,000,000 | 0.253秒 | 0.207秒 | 0.460秒 |
...
今回のコードでは、リスト作成処理と合計処理を分けて測定した。

### Bottleneck

Bottleneckは、全体の性能を制限している部分のこと。

N=10,000,000では、全体時間約0.460秒のうち、リスト作成が約0.253秒、合計処理が約0.207秒だった。

つまり、合計処理だけでなく、リストを作る処理も大きな割合を占めている。

### Practical meaning

実務では、遅いプログラムを感覚で直すのではなく、まず測定して原因を特定する。

そのため、profilingは性能改善やデバッグで重要になる。
